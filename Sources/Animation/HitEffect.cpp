...
void HitEffect::Init()
{
	...
	rapidcsv::Document clips("data_tables/animations/effect2/effect2_animation_clips.csv");
...
void HitEffect::Update(Vector2f playerPosition, float dt)
{
	effectSprite.setPosition(playerPosition);
	animation.Update(dt);
}

void HitEffect::Draw(RenderWindow& window)
{
	window.draw(effectSprite);
}

void HitEffect::SetDraw(std::string string)
{
	animation.Play(string);
}

void HitEffect::SwapScale()
{
	effectSprite.scale(-1, 1);
}

void HitEffect::SetOrigin(float x, float y)
{
